...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = /*GetWindowWidth() - 100*/ CalcItemWidth();

    const ImVec2 label_size = CalcTextSize(label, NULL, true);

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 28));

    const ImRect clickable(window->DC.CursorPos/* + ImVec2(0, 18)*/, window->DC.CursorPos + ImVec2(size, 25));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth();//GetWindowWidth();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, int tickness, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
   ...
   // const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, tickness));
    const ImRect frame_bb(total_bb.Min /*+ ImVec2(0, label_size.y + 5)*/, total_bb.Max/* + ImVec2(0, tickness)*/);
    ...
    window->DrawList->AddRectFilled(total_bb.Min, frame_bb.Max, ImColor(32, 33, 35), 5.0f);
    window->DrawList->AddRectFilled(total_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(22, 118, 243), 5.0f);

    //RenderText(total_bb.Min, label);

    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.opacity));
    {
        RenderTextClipped(total_bb.Min, total_bb.Max, value_buf, value_buf_end, NULL, ImVec2(1.f, 2.f));
    } PopStyleColor();
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, int tickness, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, tickness, format, flags);
}
